...
class DecisionTree:
    ...
    def _entropy(self, y):
        ...
        data_pts = np.sum(counts)
        entropy = 0
        if (data_pts > 0):
            ent = np.full(shape=np.size(counts), fill_value=0, dtype=float)
            probs = np.expand_dims(counts / data_pts, axis=1)
            for i in range(np.size(probs)):
                if probs[i] == 0:
                    ent[i] = 0
                else:
                    ent[i] = probs[i]*np.log2(probs[i])
            entropy = np.sum(ent)
            entropy = entropy * -1
        # ====================================================
    ...
    def _find_split(self, X, y, H_data):
        ...
        for ii in range(unique_values.shape[0] - 1):
            ...
            left_y = y[:current_split_index + 1]
            right_y = y[current_split_index+1:]

            H_left = self._entropy(left_y)
            H_right = self._entropy(right_y)
            current_information_gain = H_data - H_left*(len(left_y)/N) - H_right*(len(right_y)/N)
            if self.debug:
            ...
            if current_information_gain >= maximum_information_gain:
                # print(f"max info gain: {maximum_information_gain}")
