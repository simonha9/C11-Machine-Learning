...
class RandomForest:
    ...
    def build(self, X, y):
        ...
        for tree_i in range(self.num_trees):
            ...
            #permuting the features to use
            x_col_permute = self.rng.permutation(D)
            #permuting the data to use
            x_row_permute = self.rng.permutation(N)
            
            # print(f"x,y {X}{y}")
            #sorting the class labels based on the row permutation
            # sort_idx = np.argsort(x_row_permute)
            y_p = y[x_row_permute]
            X_p = X[x_row_permute]
            
            X_p[:,] = X_p[:,x_col_permute]
            
            X_sub = X[np.ix_(x_row_permute[:num_data_per_tree], x_col_permute[:num_features_per_tree])]
            y_sub = y_p[:num_data_per_tree]
            model.build(X_sub, y_sub)
            feat_ids = x_col_permute[:num_features_per_tree]
