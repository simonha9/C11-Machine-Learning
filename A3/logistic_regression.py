...
class LogisticRegression:
    ...
    def _compute_loss_and_gradient(self, X, y, alpha_inverse=0, beta_inverse=0):
        ...
        #y is yk for our labels

        #stack 1 in X for bias
        ones = np.ones(shape=(N,1))
        X = np.hstack((ones, X))

        #Create sig for logits
        sig = X @ self.parameters.T
        #Get softmax for these, soft = (N,K)
        softm = softmax(sig)
        soft = np.log(softm)

        # C-1
        alpha = np.empty(D)
        alpha.fill(alpha_inverse)
        c_d = np.insert(alpha, 0, beta_inverse)
        C = np.diag(c_d)
        #grad = (K,D+1)
        grad = np.empty(shape=(self.num_classes, D+1))
        #l2 grad = (K,D+1)
        l2 = self.parameters @ C 
        
        #l2_nll = (K,K)
        l2_nll = self.parameters @ C @ self.parameters.T

        #1hot = (N,K)
        y_ik = np.zeros(shape=(N,self.num_classes))
        for k in range(self.num_classes):
            y_one = np.where(y==k)[0]
            y_ik[y_one, k] = 1

        #E1_nll element wise multiplication = (N,K)
        E1_nll = np.multiply(y_ik,soft)
        #y-soft = (N,K)
        E1_d = np.subtract(y_ik, softm)

        nll = -np.sum(E1_nll)/N + np.sum(l2_nll)   
        for j in range(self.num_classes):
            E1_dcol = np.array([E1_d[:,j]])
            grad_term = np.array(np.multiply(E1_dcol.T, X))
            E_d = -np.sum(grad_term, axis=0)
            grad[j] = (E_d/N) + C @ self.parameters[j]
