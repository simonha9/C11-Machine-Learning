...
class GMM:
    ...
    def _e_step(self, train_X):
        ...
        #for each data pt do prob_matrix[i][j] = p(l = j| train_X[i], theta)
        # for i in range(N):
        for j in range(self.K):
            m_j = self.mixture_proportions[j]
            mu_j = self.centers[j, :]
            C_j = self.covariances[j]

            # NxD - D
            diff = np.subtract(train_X, mu_j)
            # print(f"train_X: {train_X.shape}")
            # print(f"mu_j: {mu_j.shape}")
            # print(f"diff: {diff.shape}")
            # print(f"C_j: {C_j.shape}")
            p1 = diff @ (np.linalg.inv(C_j)) @ diff.T
            # print(f"p1: {p1.shape}")
            exp = np.diag(p1)
            const = 1 / (np.sqrt(((2 * np.pi) ** D) * np.linalg.det(C_j)))
            gamma = np.multiply(const * m_j, np.exp(-0.5 * exp))
            probability_matrix[:, j] = gamma
        #divide by sum of column
        probability_matrix = np.divide(probability_matrix, probability_matrix.sum(axis=1, keepdims=True))
        # ====================================================
    ...
    def _m_step(self, train_X, probability_matrix):
        ...
        # TODO: Implement your solution within the box
        for j in range(self.K):
            col = probability_matrix[:, j]
            total_sum = np.sum(col)
            m_j = total_sum / N
            
            # mu_j
            x_sum = np.sum(np.array(col)[:, np.newaxis] * train_X, axis=0)
            mu_j = np.divide(x_sum, total_sum)

            # C_j
            x_cov_sum = (np.subtract(train_X, np.array(mu_j)[np.newaxis, :]))
            outer = np.dot(np.multiply(x_cov_sum, col[:, np.newaxis]).T, x_cov_sum)
            C_j = np.divide(outer, total_sum)
            
            mixture_proportions[j] = m_j
            centers[j] = mu_j
            covariances[j] = C_j
        # ====================================================
