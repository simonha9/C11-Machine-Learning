...
class PCA:
    ...
    def plot_eigenvalues(self, savefig=False):
        ...
        if savefig:
            ...
            # plt.savefig(f"results/eigenvalues.png", format="png")
        else:
            plt.show()
        plt.clf()
    ...
    def plot_subspace_variance(self, savefig=False):
        ...
        # TODO: Implement your solution within the box
        eigenvalues = self.w
        k = np.arange(0, self.D+1)
        total_sum = np.sum(eigenvalues)
        fractions = np.empty(shape=(self.D + 1,))
        k_len = len(k)
        for i in range(k_len):
            fractions[i] = (np.sum(eigenvalues[:k[i]])) / total_sum
        plt.plot(fractions, k)
        ...
        if savefig:
            ...
            # plt.savefig(f"results/fraction_variance.png", format="png")
        else:
            plt.show()
        plt.clf()
