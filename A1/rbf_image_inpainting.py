"""
Script originally provided by CSCC11 profs.
...
The user can choose a color to fill in, then the algo iterates through each section (image is partitiioned into sections). 
Look at pixels that are similar to the filled in pixel (by eucl dist), train the RBF model by using diff pixels (to optimize the weights)
and evaluate the RBF by filling in similar pixels with these weights.
...
def RBF_image_inpainting(image_name, fill_rgb, spacing, width, l2_coef, patch_size=25, tolerance=0.1):
    ...
    Args:
    - image_name (str): The image to be inpainted.
